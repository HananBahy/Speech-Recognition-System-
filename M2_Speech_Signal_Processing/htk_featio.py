...
def read_htk_user_feat(name='filename'):
    ...
    #print( num_samples)   #num_samples is number of frames
    if parm_kind != 9:
    ...
    feat = np.zeros([num_dim,num_samples],dtype=float)   ###changed
    for t in range(num_samples):
        feat[:,t] = np.array(struct.unpack('>' + ('f' * num_dim), f.read(samp_size)),dtype=float)  ###changed
    #print(feat.shape)
    #feat=feat.T
    return feat

...
class FrontEnd:
    ...
    def __init__(self, samp_rate=16000, frame_duration=0.025, frame_shift=0.010, preemphasis=0.97,
        ...
        self.hamwin = np.hamming(self.win_size)    #hamming window
    ...
    def make_mel_filterbank(self):
        ...
        for i in range(0,self.num_mel):
            left_bin = int(mel_bins[i])  #left
            center_bin = int(mel_bins[i+1])  #center
            right_bin = int(mel_bins[i+2])    #right
    ...
    def pre_emphasize(self, wav):
        ...
        #The pre-emphasis filter can be applied to a signal x using the first order filter in the following equation:
          #y(t)=x(t)−αx(t−1)
         ###the filter coefficient (α) are 0.95 or 0.97, pre_emphasis = 0.97:
        preemph_wav = []
        preemph_wav = np.append(wav[0], wav[1:]-self.preemphasis*wav[0:-1])
        return preemph_wav
    ...
    def wav_to_frames(self, wav):
        ...
        for t in range(0, num_frames):
            ...
            frames[:, t] = self.hamwin * frame   #apply hamming window to each frame
    ...
    def frames_to_magspec(self, frames):
       ...
       # print("///////////////")
        #print(frames.shape)
        Nftt=513 #to meet the shape of self.mel_filterbank.T ,where magspec.shape ==frames.shape[0]xceil(Nftt/2)
        magspec =np.absolute(np.fft.rfft(frames,Nftt,axis=0))   #Compute the one-dimensional discrete Fourier Transform for real input.
                                                                                #compute along the last axis (columns)                                           
        #print(magspec.shape)
        return magspec
    ...
    def magspec_to_fbank(self, magspec):
        ...
        #pow_frames = ((1.0 / 513) * ((magspec) ** 2))
        #print("///////////////")
        #print(magspec.shape)
         
        fbank =np.dot(self.mel_filterbank ,magspec) #applying the filter bank tothe magnitude spectrum
    ...
    def mean_norm_fbank(self, fbank):  #Mean normalization
